...
public class DBF
   ...
   implements DBFInterface
  {
	...
	private void setInterfaceFieldsFromDBF()
  		throws RemoteException
	{
		try {
	    	for (int i = 0; i < dbf.getFieldCount(); i++)
	  		  fields[i].put(dbf.getField(i+1).get());
			}
			catch (xBaseJException xe) {
				throw new RemotexBaseJException( xe.getMessage());
			}
	...
	private void putInterfaceFieldsToDBF()
  		throws RemoteException
	{
		try {
    	for (int i = 0; i < dbf.getFieldCount(); i++)
  		  dbf.getField(i+1).put(fields[i].get());
			}
			catch (xBaseJException xe) {
				throw new RemotexBaseJException( xe.getMessage());
			}
  	...
  	public FieldInterface  getField(String name)
  		throws RemoteException
  		{
			...
			for (int i = 0; i < dbf.getFieldCount(); i++)
				  if (fields[i].getName().toLowerCase().compareTo(lcn) == 0)
				     return fields[i];
