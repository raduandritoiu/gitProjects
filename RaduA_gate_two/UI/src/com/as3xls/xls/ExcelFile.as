...
//		public function saveMultimpleToByteArray():ByteArray 
//		{
//			var br:BIFFWriter = new BIFFWriter();
//			
...
//			
//			// Write the BOF and header records
//			var bof:Record = new Record(TypeExt.BOF_BIFF5);
//			bof.data.writeShort(0x0500); 				// BIFF version
//			bof.data.writeShort(GLOBALS); 				// type of data in stream
//			bof.data.writeShort(0x01);					// Build identifier
//			bof.data.writeShort(0x01);					// build year
//			br.writeTag(bof);
//			
//			// Window1 - added by me
//			var wndw:Record = new Record(Type.WINDOW1);
//			wndw.data.writeShort(0); 					// Horizontal position
//			wndw.data.writeShort(0); 					// Vertical position
//			wndw.data.writeShort(0xc8); 				// Width
//			wndw.data.writeShort(0xc8);					// Height
//			wndw.data.writeShort(0); 					// Option flags
//			wndw.data.writeShort(0);					// Index to active (displayed) worksheet
//			wndw.data.writeShort(0);					// Index of first visible tab in the worksheet tab bar
//			wndw.data.writeShort(0);					// Number of selected worksheets
//			wndw.data.writeShort(0xff);					// Width of worksheet tab bar in percentages
//			br.writeTag(wndw)
//				
//			// Date mode
//			var dateMode:Record = new Record(Type.DATEMODE);
//			dateMode.data.writeShort(1); 				// Base date flag
//			br.writeTag(dateMode);
//			
//			// fonts - lets not add fonst now ad see how that works- TODO
//			
//			// Store built in formats - do not write formts - see what happens
////			var formats:Array = ["General", 
////				"0", "0.00", "#,##0", "#,##0.00", 
////				"", "", "", "",
////				"0%", "0.00%", "0.00E+00",
...
////				"M/D/YY", "D-MMM-YY", "D-MMM", "MMM-YY"];
//			
//			// do not write the Built_in_FMT_count - TODO
////			var numfmt:Record = new Record(Type.BUILTINFMTCOUNT);
////			numfmt.data.writeShort(formats.length);
////			br.writeTag(numfmt);
//			
...
////			}
//			
//			// XF - lets not add fonst now ad see how that works- TODO
//			// STYLE - lets not add fonst now ad see how that works- TODO
//			
//			// Sheets
//			for (var i:int = 0; i < 1; i++) {
//				var s:Sheet = _sheets[i] as Sheet;
//				var sheet:Record = new Record(TypeExt.SHEET);
//				sheet.data.writeInt(0x0); 				// stream position 
//				sheet.data.writeByte(0x0); 				// Visible
//				sheet.data.writeByte(0x0);				// Worksheet
//				var len:uint = s.name.length;
//				sheet.data.writeByte(len); 				// Name length
//				sheet.data.writeUTFBytes(value); 		// Name
//				br.writeTag(sheet);
//			}
//			
//			// The closing EOF record - for Wrokbook globals
//			var eof:Record = new Record(Type.EOF);
//			br.writeTag(eof);
//
//			
...
//				var bof:Record = new Record(TypeExt.BOF_BIFF5);
//				bof.data.writeShort(0x0500); 			// BIFF version
//				bof.data.writeShort(SHEET); 			// type of data in stream
//				bof.data.writeShort(0x01);				// Build identifier
//				bof.data.writeShort(0x01);				// build year
//				br.writeTag(bof);
//	
//				// Dimensions
//				var dimensions:Record = new Record(TypeExt.DIMENSION_BIFF5);
//				dimensions.data.writeShort(0);			// Index first row
//				dimensions.data.writeShort(s.rows+1); 	// Index last row
//				dimensions.data.writeShort(0);			// Index first col
//				dimensions.data.writeShort(s.cols+1);	// Index last row
//				dimensions.data.writeShort(0);			// Not used
//				br.writeTag(dimensions);
//				
//				for (var r:uint = 0; r < s.rows; r++) 
//				{
...
//		}
		...
		// Handler functions that handle reading the Values from records
