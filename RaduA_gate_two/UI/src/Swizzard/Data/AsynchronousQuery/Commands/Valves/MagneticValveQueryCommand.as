package Swizzard.Data.AsynchronousQuery.Commands.Valves
{
	...
	import j2inn.Data.Query.Interfaces.IQueryItem;
	...
	public class MagneticValveQueryCommand extends BaseValveQueryCommand
	{
		private static const ACTUATOR_REQUERY_FIELDS:Array	= ["signal", "supplyVoltage", "endSwitch", "positioner", "hasHeater",
																"motor", "safetyFunction"];
		
		public function MagneticValveQueryCommand()
		{
			super();
			valveType	= ValveType.MAGNETIC;
		}
		...
		override public function execute(notification:INotification):void
		{
			...
			// If incompatible actuator
			if (shouldSkip || ((t.target == ProductQueryType.ACTUATORS) && !t.requeryAllValves) ||
				(queryModel.actuator.motor > ActuatorMotorType.ELECTRIC) || 
				((queryModel.actuator.signal != -1) && (queryModel.actuator.signal != ActuatorControlSignal.s0_10_VDC) && 
				(queryModel.actuator.signal != ActuatorControlSignal.s4_20mA)) ||
				((queryModel.actuator.safetyFunction != ActuatorSafetyFunction.SPRING_RETURN) && (queryModel.actuator.safetyFunction > 0)) || (queryModel.actuator.supplyVoltage == 120)) 
			{
				skip();
				return; // Magnetic Valves have no actuator selection
			}
		...
		override protected function generateValveQuery():SQLDataQuery
		{
			...
			if (form.pattern > 0)
			{
				var patternItem:IQueryItem = SQLQueryItem.Build("pattern", QueryOperations.EQUAL_TO, form.pattern, "t");
				if (form.pattern != ValvePattern.SIX_WAY) {
					var patternGroup:QueryGroup	= new QueryGroup();
					patternGroup.predicate = QueryPredicates.AND;
					patternGroup.addItem(patternItem);
					patternGroup.addItem(SQLQueryItem.Build("pattern", QueryOperations.EQUAL_TO, ValvePattern.BOTH, "t", QueryPredicates.OR));
					patternItem = patternGroup;
				}
				query.addItem(patternItem);
			}
			...
			LogUtils.ValveSQL("Magnetic SQL: " + query.toSql());
			return query;
		}
		...
		override protected function generateActuatorQuery():SQLDataQuery
		{
			...
			var form:ActuatorQueryModel	= token.valveQueryModel.actuator;
