package Swizzard.System.Models
{
	...
	[Bindable]
	[RemoteClass]
	public class DamperScheduleItem extends BaseScheduleItem
	{
		public static const FILE_EXTENSION_NAME:String = "sdf";
		
		
		public function DamperScheduleItem() {
			super();
		}
		...
		override public function set product(value:AbstractSiemensProduct):void {
			if ((value is SiemensDamper) || (value is SiemensAccessory)) {
				// Got Item
				_product = value;
			}
			else {
				...
				var productType:uint = ExtendedProductType.toDamperDimensionType(value.extendedProductType);
		...
		override public function clone(withParent:Boolean = true):IScheduleItem {
			var c:DamperScheduleItem = new DamperScheduleItem();
			c.product			= product;
			c.notes				= notes;
			c.quantity			= quantity;
		...
		public function addSubItem(item:DamperScheduleItem):void {
			item._parent = this;
		...
		override flash_proxy function getProperty(name:*):* {
			...
			else if (discriminant == "damper" && product is SiemensDamper) {
				return product[propertyName];
			}
		...
		override public function readExternal(input:IDataInput):void {
			...
			for each (var item:DamperScheduleItem in subitems) {
				item._parent = this;
			}
		}
	}
