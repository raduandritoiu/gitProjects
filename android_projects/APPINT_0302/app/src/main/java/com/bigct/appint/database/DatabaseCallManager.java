...
public class DatabaseCallManager
{
	...
	public void printRunningTasks()
	{
		for(DatabaseCallTask task : mTaskList)
		{
			Log.d("DB", "DatabaseCallManager.printRunningTasks(): " + (task == null ? "null" : (task.getQuery().getClass().getSimpleName() + " / " + task.getStatus().toString())));
		}
		
		if(mTaskList.isEmpty()) Log.d("DB", "DatabaseCallManager.printRunningTasks(): empty");
	}
}
