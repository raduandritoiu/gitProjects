...
//			if (_posPoint1 == null) {
//				_posPoint1 = point;
//				_l1 = Math.sqrt((_x - point.x) * (_x - point.x) + (_y - point.y) * (_y - point.y));
//			}
//			else if (_posPoint2 == null) {
//				_posPoint2 = point;
//				_l2 = Math.sqrt((_x - point.x) * (_x - point.x) + (_y - point.y) * (_y - point.y));
//			}
//			else return;
...
//			var l:Number = (_posPoint1 == point) ? _l1 : _l2;
		...
		protected function posPointMovedIsMobile(evt:PointEvent):void {
			if (_posPoint1 == null || _posPoint2 == null) {
				mobileOnePoint(evt);
				return;
			}
		...
		protected function mobileOnePoint(evt:PointEvent):void {
			...
			var alfa:Number = CalculusUtils.arctan(_x, _y, _posPoint1.x, _posPoint1.y);
			
			var newX:Number = _posPoint1.x + _l1 * Math.cos(alfa);
			var newY:Number = _posPoint1.y + _l1 * Math.sin(alfa);
			var newX_tmp:Number = _posPoint1.x - _l1 * Math.cos(alfa);
			var newY_tmp:Number = _posPoint1.y - _l1 * Math.sin(alfa);
