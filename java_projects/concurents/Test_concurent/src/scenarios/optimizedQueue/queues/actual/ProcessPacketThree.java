...
import scenarios.optimizedQueue.packets.Packets.PacketThree_act;
import scenarios.optimizedQueue.queues.IQueue.IInsertElem;
import utils.Utils;


public class ProcessPacketThree implements IPacketProcessor {
  public int pkt_list_in = 0;
  public int pkt_list_out = 0;
  public final int pkt_lst_size;
  public final PacketThree_act[] pkt_list;
  
  public ProcessPacketThree(int size) { 
    pkt_lst_size = size;
    pkt_list = new PacketThree_act[pkt_lst_size];
    for (int i = 0; i < pkt_lst_size; i++)
      pkt_list[i] = new PacketThree_act();
  }
  
  public String name() { return "\t\t ProcessPacketThree"; }
  public String toString() { return "\t\t ProcessPacketThree"; }
...
//===================================================================
  public boolean acceptPacket(IInsertElem inPkt) {
    ...
    if (advance(pkt_list_in) != pkt_list_out) {
      ...
      if (pkt_list[pkt_list_in].info_three != Packets.empty_msg) {
        System.out.println(name() + " - producer: WTF!!!");
        Utils.CRASH();
      }
      // add the packet
      pkt_list[pkt_list_in].info_three = ((InputPacket)inPkt).getInMsg();
  ...
  public void processPacket() {
    if (pkt_list_out != pkt_list_in) {
      ...
      if (pkt_list[pkt_list_out].info_three == Packets.empty_msg) {
      ...
      pkt_list[pkt_list_out].info_three = Packets.empty_msg;
